...
document.onreadystatechange = function () {
  ...
  async function renderApp() {
    try {
      ...
      apiKey = iparams.api_key||"cmJnOLEb9GhM4gkQ-EE5"; // Use correct key from iparams
...
// Make fetchTicketDetails globally accessible
window.fetchTicketDetails = async function() {
  ...
  // Get domain from iparams
 

  const url = `https://workdeft199.freshservice.com/api/v2/tickets/${ticketId}`;
  const headers = {
  ...
  try {
    ...
    ticketOutput.innerHTML = `
      ...
      🔧 <strong>Status:</strong> ${ticket.ticket.status} &nbsp;|&nbsp; <strong>Priority:</strong> ${ticket.ticket.priority}
    `;
  } catch (error) {
